...
class RouteServiceTest {
    ...
    void update_success() throws Exception {
        var newDest = buildLocation(3L, "HUSE Aracaju", -10.94380, -37.07150);
        when(routeRepository.findById(1L)).thenReturn(Optional.of(route));
        when(locationRepository.findById(1L)).thenReturn(Optional.of(origin));
        when(locationRepository.findById(3L)).thenReturn(Optional.of(newDest));
        when(routeRepository.existsByOriginAndDestination(origin, newDest)).thenReturn(false);

        DetailRoute result = routeService.update(1L, new UpdateRoute(1L, 3L, 4L));
